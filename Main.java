import java.util.Scanner;

public class Main {

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        String texto = sc.nextLine();
        while (texto.equals("FIM") == false) {
            String X1 = soVogais(texto) ? "SIM" : "NAO";
            String X2 = soConsoantes(texto) ? "SIM" : "NAO";
            String X3 = soNumero(texto) ? "SIM" : "NAO";
            String X4 = soNumeroReal(texto) ? "SIM" : "NAO";

            System.out.println(X1 + " " + X2 + " " + X3 + " " + X4);
            texto = sc.nextLine();
        }

    }

    public static boolean soVogais(String entrada) {
        if (entrada.length() == 0) {
            return true;
        }
        char c = Character.toLowerCase(entrada.charAt(0));
        if (c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u') {
            return soVogais(entrada.substring(1));
        } else {
            return false;
        }
    }

    public static boolean soConsoantes(String entrada) {
        if (entrada.length() == 0) {
            return true;
        }
        char c = Character.toLowerCase(entrada.charAt(0));
        if ((c >= 'a' && c <= 'z') && !(c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u')) {
            return soConsoantes(entrada.substring(1));
        } else {
            return false;
        }
    }

    public static boolean soNumero(String entrada) {
        if (entrada.length() == 0) {
            return true;
        }
        char c = entrada.charAt(0);
        if (c >= '0' && c <= '9') {
            return soNumero(entrada.substring(1));
        } else {
            return false;
        }
    }

    public static Boolean soNumeroReal(String entrada) {
        return soNumeroReal(entrada, 0, 0);
    }

    private static Boolean soNumeroReal(String entrada, int countDot, int countComma) {
        if (entrada.length() == 0) {
            return true;
        }
        char c = entrada.charAt(0);
        if (c == '.') {
            countDot++;
        }
        if (c == ',') {
            countComma++;
        }
        if (countDot > 1 || countComma > 1) {
            return false;
        }

        return false;
    }


}